...
global   funktionscount zeit max_abstand
...
options=optimset('Algorithm','trust-region-reflective','TolFun',1e-12,'MaxFunEvals',10000,'MaxIter',10000,'Display','off');   % Option to display output

clk=clock;
for j=1:10
    [koord,~,~,output] = fsolve(@GLS,koord_start,options);  % Call optimizer
    C_=clock;
